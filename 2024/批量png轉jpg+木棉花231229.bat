...
set dir_orig=%nnn%_orig
mkdir %dir_orig%
set dir_jpg=%nnn%_jpg
mkdir %dir_jpg%
...
for %%a in (*.png) do (
  set /a counter+=1
  set name=測試%nnn%_!counter!_!RANDOM!.jpg
  echo !name!
  E:\_ffmpeg\_ffmpeg\ffmpeg.exe -i "%%a" -q:v 3  %vf% -y !name!
  move "%%a" ./%dir_orig%/
  move !name! ./%dir_jpg%/
)
...
-vf "scale=800:450,setsar=1/1"


mkdir %nnn%_webp
for %%a in (*.webp) do (
  E:\_ffmpeg\_ffmpeg\ffmpeg.exe -i %%a -q:v 3 -y "測試%nnn%_!counter!.jpg"
  set /a counter+=1
  move %%a ./%nnn%_webp/
)
...
powershell Get-Date -Format "yyMMdd_HHmmss"
for /f %%A in ('powershell Get-Date -Format "yyMMdd_HHmmss"') do set "nnn=%%A"
...
ffmpeg -i %input% -q:v 3 -y output.jpg
